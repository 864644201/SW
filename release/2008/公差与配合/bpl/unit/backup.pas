...
type
  ...
  public
    ...
    function  RestoreFile(Source: String; TargetFile: string): boolean;
...
begin
     ...
     repeat
        ...
        //   DoRestore:=ExtractFileName(S)=ExtractFileName(CurrentFile);
...
function  TBackupFile.RestoreFile(Source: String; TargetFile: string): boolean;
var
   L: Integer;
   FA, FAT: integer;
   S, Disk: String;
...
begin
     ...
     repeat
        ...
        begin
          ...
          //  if fRestoreFullPath and (TargetPath <> '') then CurrentFile := TargetPath + copy(S, 4, Length(S));
           ...
           DoRestore:=UpperCase(ExtractFileName(S))=UpperCase(ExtractFileName(CurrentFile));
     ...
     if assigned(fOnProgress) then fOnProgress(self, '', 100, Continue);
     IsBusy := false;
     if fLastErr <> 0 then MessageError(fLastErr);end;
